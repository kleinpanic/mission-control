...
**Status**: [x] Complete (reduced to 144)
**Risk**: Low (code quality, no behavior change)  
**Focus areas** (from lint output):
- [x] Refactor `src/types/index.ts` to use `unknown` instead of `any`
- [x] Refactor `src/app/api/agents/route.ts`
- [x] Refactor `src/lib/db.ts`
- [x] Refactor other API routes (status, costs, sessions, config, tasks, cron, rate-limits)
- [x] Refactor main dashboard page (`src/app/page.tsx`)
- [x] Fix component render issues (`src/components/costs/ModelUsageChart.tsx`)
- [x] Fix React hook warnings in Header, QuickStats, Sidebar, and BudgetAlerts
- [x] Prioritize: API routes, types/index.ts, components with most warnings

### 3. Verify CI passes with changes
**Status**: [/] In progress  
**Risk**: Low  
**Actions**:
- [x] Run `pnpm lint` locally (passed with 144 warnings)
...
- [x] CI workflow uses version-agnostic better-sqlite3 rebuild
- [x] Lint warnings reduced to <150 (from 213)
